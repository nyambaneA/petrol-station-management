    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const [fuelResponse, salesResponse, expensesResponse] = await Promise.all([
          axios.get(`https://localhost:5000/api/fuel/remaining?date=${date}`, { headers: { Authorization: `Bearer ${token}` } }),
          axios.get(`https://localhost:5000/api/sales/total?date=${date}`, { headers: { Authorization: `Bearer ${token}` } }),
          axios.get(`https://localhost:5000/api/expenses/date?date=${date}`, { headers: { Authorization: `Bearer ${token}` } })
        ]);
